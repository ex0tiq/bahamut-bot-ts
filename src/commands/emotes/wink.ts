...
const config: CommandConfig = {
    ...
    testOnly: false,
    description: "You are winking [at someone]",
    minArgs: 0,
...
export default {
    ...
    callback: async ({ client, message, channel, member, args, interaction }: { client: BahamutClient, message: Discord.Message, channel: Discord.TextChannel, member: Discord.GuildMember, args: any[], interaction: Discord.CommandInteraction }) => {
        ...
        try {
            const res = await client.bahamut.tenor.Search.Query("wink", "30");
            ...
            if (target) {
                return handleResponseToMessage(client, message || interaction, false, config.deferReply, {
                    embeds: [
                        new Discord.EmbedBuilder()
                            .setDescription(await lang.getMessage(client, channel.guild, "commands_emotes_wink_target_text", {
                ...
                return handleResponseToMessage(client, message || interaction, false, config.deferReply, {
                    embeds: [
                        new Discord.EmbedBuilder()
                            .setDescription(await lang.getMessage(client, channel.guild, "commands_emotes_wink_text", {
                                user1: member.toString(),
                            }) || "")
                            .setImage(post.media[0].gif.url)
                            .setFooter({ text: "Via Tenor" }),
