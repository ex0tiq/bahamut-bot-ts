...
export default {
    ...
    callback: async ({
                         ...
                         member,
                         interaction,
                     }: { client: BahamutClient, message: Discord.Message, channel: Discord.TextChannel, member: Discord.GuildMember, interaction: Discord.CommandInteraction }) => {
        const settings = await getGuildSettings(client, channel.guild);
        // Abort if module is disabled
        if (settings.disabled_categories.includes("ffxiv")) return;

        const ffCharId = await client.bahamut.dbHandler.ffxiv.getDBGuildFFXIVCharacterID(channel.guild, member),
            xiv = new XIVAPI({
                private_key: client.bahamut.config.xivapi_token,
                language: settings.language,
            }), jobEmojiList = client.bahamut.config.job_emoji_list;
        let response = null, totalMinions = [], totalMounts = [];
        ...
        if (response) {
            ...
            try {
                ...
                if ((response.Character.Race && response.Character.Race.Name) && (response.Character.Tribe && response.Character.Tribe.Name)) {
                    msg.addFields({
                        name: "Race and Tribe",
                        value: `${response.Character.Race.Name}, ${response.Character.Tribe.Name}`,
                        inline: true,
                    });
                } else {
                ...
                if (response.Character.GuardianDeity && response.Character.GuardianDeity.Name) {
                    msg.addFields({
                        name: "Guardian Deity",
                        value: response.Character.GuardianDeity.Name,
                        inline: true,
                    });
                } else {
                ...
                if (response.Character.FreeCompanyName || (response.Character.GrandCompany && response.Character.GrandCompany.Company && response.Character.GrandCompany.Company.Name)) {
                    if (response.Character.FreeCompanyName) {
                        msg.addFields({
                            name: "Free Company",
                            value: response.Character.FreeCompanyName,
                            inline: true,
                        });
                    } else {
                    ...
                    if (response.Character.GrandCompany && response.Character.GrandCompany.Company && response.Character.GrandCompany.Company.Name) {
                        msg.addFields({
                            name: "Grand Company",
                            value: response.Character.GrandCompany.Company.Name,
                            inline: true,
                        });
                    } else {
                ...
                if (response.Mounts && response.Mounts.length > 0 && totalMounts && totalMounts.length > 0) {
                    msg.addFields({
                        name: "Mounts",
                        value: `${response.Mounts.length}/${totalMounts.length} (**${Math.round((response.Mounts.length * 100) / totalMounts.length)}%**)`,
                        inline: true,
                    });
                } else {
                ...
                if (response.Minions && response.Minions.length > 0 && totalMinions && totalMinions.length > 0) {
                    msg.addFields({
                        name: "Minions",
                        value: `${response.Minions.length}/${totalMinions.length} (**${Math.round((response.Minions.length * 100) / totalMinions.length)}%**)`,
                        inline: true,
                    });
                } else {
                ...
                if (response.Character.ClassJobs && response.Character.ClassJobs.length > 0) {
                    ...
                    if (dslText !== "" || dshText !== "") {
                        msg.addFields({
                            name: "Disciples of the Land/Hand",
                            value: `${((dslText !== "" ? `${dslText.replace(/\n$/, "")}\n\n` : ""))}${((dshText !== "" ? `${dshText.replace(/\n$/, "")}\n\n` : ""))}`,
                            inline: true,
                        });
                    }
                }
